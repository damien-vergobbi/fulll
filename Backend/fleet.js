...
program
    ...
    .action(async (fleetId, vehiclePlateNumber) => {
        try {
            ...
            if (!fleet) {
                console.error(`Error: No fleet found for user ${fleetId}. Use 'create' first.`);
                process.exit(1);
            }
...
program
    ...
    .action(async (fleetId, vehiclePlateNumber, latitude, longitude, altitude) => {
        try {
            const fleet = await fleetRepository.findByUserId(fleetId);
            if (!fleet) {
...
program
    .command('list')
    .description('[Debug command] Lists all fleets and their vehicles with locations.')
    .action(async () => {
        try {
            ...
            fleets.forEach(fleet => {
                ...
                if (vehicles.length > 0) {
                    ...
                    vehicles.forEach(entry => {
                        ...
                        if (entry.location) {
                            console.log(`      Position: lat: ${entry.location.latitude}, long: ${entry.location.longitude}${entry.location.altitude ? `, alt: ${entry.location.altitude}` : ''}`);
                        }
                    });
                } else {
